{
  keymapGroup =
    prefix: keymaps:
    let
      adaptKeymap = { key, ... }@keymap: keymap // { key = "${prefix}${key}"; };
    in
      ...
      prepareKeymaps =
        ...
        let
          ...
          turnOffKey = {
            ...
            options = options // {
              desc = "Turn off ${spec.description}";
            };
          };
        in
        [ toggleKey ]
        ++ (
          if actions ? turnOn then
            [
              turnOnKey
              turnOffKey
            ]
